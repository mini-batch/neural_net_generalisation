...
def extend_params(params : list[torch.Tensor],
    ...
    params[2] = torch.cat([params[2], torch.normal(0, 0.01, size=(10, hidden_unit_diff), device=device)], dim=1)
...
#region Training and metric utils
...
# Start models will be from the convergence script, as each model of size 20 will have reached steady state
# in train and test loss.
