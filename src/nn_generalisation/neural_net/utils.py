...
def extend_params(params : list[torch.Tensor],
    ...
    params[0] = torch.cat([params[0], torch.zeros((hidden_unit_diff, 784)).to(device)], dim=0)
    assert params[0].shape == (new_model_size, 784), f"Expected {(new_model_size, 784)} but got {params[0].shape}"
    params[1] = torch.cat([params[1], torch.zeros((hidden_unit_diff)).to(device)], dim=0)
    assert params[1].shape[0] == (new_model_size), f"Expected {new_model_size} but got {params[1].shape}"
    params[2] = torch.cat([params[2], torch.zeros(10, hidden_unit_diff).to(device)], dim=1)
